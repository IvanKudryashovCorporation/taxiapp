#!/bin/bash
# Сборка APK приложения пассажира для Android.
# Запускать из WSL: bash passenger_app/build_android.sh
set -e
...
echo "==> Сборка: Рассвет — Пассажир"
echo "    Источник: $TAXIAPP_DIR"

# Создаём временный main.py в корне taxiapp/
cat > "$MAIN_PY" << 'PYEOF'
from passenger_app.app import PassengerMobileApp
...
# Запускаем buildozer из директории passenger_app/
cd "$SCRIPT_DIR"
